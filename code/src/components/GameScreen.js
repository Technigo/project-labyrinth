...
const GameScreen = () => {
  ...
  return (
    <div>
      ...
      {/* <button disabled={!history.length} onClick={onDescriptionRevert}>
        Go Back
      </button>
      <h3>This is a quote : {currentPosition.description}</h3>
      {currentPosition.direction.map((direction) => (
        <button
          key={direction}
          onClick={() => onDescriptionRegenerate(direction)}
        >
          {direction}
        </button>
      ))} */}
